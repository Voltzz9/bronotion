...
export function FloatingPanelForm({
  ...
  const { tag, closeFloatingPanel } = useFloatingPanel()
...
export function FloatingPanelLabel({
  ...
  const { tag } = useFloatingPanel()

  return (
    <motion.label
      htmlFor={htmlFor}
      style={{ opacity: tag ? 0 : 1 }}
      className={cn(
...
export function FloatingPanelTextarea({
  ...
  const { tag, setTag } = useFloatingPanel()
  ...
  return (
    <div className="flex justify-center items-center h-full">
      <textarea
        id={id}
        className={cn(
          "w-32 h-8 resize-none rounded-md bg-transparent px-1 py-1 text-sm outline-none",
          className
        )}
        autoFocus
        value={tag}
